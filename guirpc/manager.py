...
cli_manager = click.CommandCollection(
    ...
    "This is a command line manager to start, "
    "run and test RPC consumers and producers.",
...
@click.option(
    "-E", "--exchange", help="the exchange name.", default="rpc_gateway"
)
@click.option(
    "-t", "--exchange-type", help="the exchange type.", default="direct"
)
@click.option("-q", "--queue", help="the queue name.", default="my_queue")
@click.option(
    "-r", "--routing-key", help="the routing key.", default="my_queue"
)
@click.option(
    "-p",
    "--prefetch-count",
    type=int,
    help="the QOS prefetch count.",
    default=1,
)
@click.option(
    "-w",
    ...
    "(each worker can handle 1 message at a time)",
    default=4,
)
@click.option(
    "-U",
...
@click.argument("app_name")
@click.option(
    "-p", "--producer-id", help="the application producer id.", default=""
)
@click.option(
    "-E", "--exchange", help="the exchange name.", default="rpc_gateway"
)
...
@click.option(
    "-U",
    "--connect",
    help="the RabbitMQ server url to connect to "
...
@click.option(
    "--client",
    is_flag=True,
    help="With this flag it generates "
    "a client configuration file. Defaults server.",
)
...
def create_config(name, **options):
    """Creates a server or client INI configuration file
    with default values."""
