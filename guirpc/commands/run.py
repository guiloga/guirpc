...
from guirpc.amqp.domain.exceptions import (
    AMQPConnectionURINotSetError,
...
def find_registered_faas(app_file):
    ...
    if not err:
        ...
        for name in attr_names:
            ...
            try:
                if (
                    attr_.__qualname__ == "register_faas."
                    "<locals>.exec_wrapper.<locals>._exec"
                ):
...
class ConfigININotProvidedError(Exception):
    def __str__(self):
        return (
            "The .ini configuration file has not been provided. "
...
def runconsumer(with_config, **options):
    ...
    if not err:
        ...
        logger.info(
            "Options (throughput): max_workers=%s; prefetch_count=%s;"
