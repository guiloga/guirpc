...
def initconsumer(work_dir, app_name, **options):
    _init_cmd_handler("consumer", work_dir, app_name, CONSUMER_INI, options)


def initproducer(work_dir, app_name, **options):
    _init_cmd_handler("producer", work_dir, app_name, PRODUCER_INI, options)
...
def createconfig(name, **options):
    ...
    print(
        "Creating {0} config INI named '{1}.ini'..".format(
            "client" if client_flag else "server", name
        )
    )
    ...
    try:
        if client_flag:
            values = {
                **CONFIG_DEFAULTS["producer_config"]["globals"],
            ...
            values = {
                **CONFIG_DEFAULTS["consumer_config"]["amqp_entities"],
...
def _create_config_file(
    name: str, config_ini: str, format_values: dict, out_dir: str = ""
):
    ...
    config.read_string(config_ini.format(app_name=name, **format_values))
